...
//     DashboardItem(title: 'Teacher', icon: Icons.admin_panel_settings, page: Admin()),
//   ];
...
//       print("userMap");
//     final String? name = prefs.getString('orgName');
//   final String? email = prefs.getString('Email');
//   final String? registerNumber = prefs.getString('RegisterNumber');
//   final int? organizationID = prefs.getInt('ID');

//   print('Name: $name');
//   print('Email: $email');
//   print('Register Number: $registerNumber');
//   print('Organization IDs: $organizationID');
//   print(organizationID.runtimeType);
//   final int? organisationId = _userData['ID'] != null ? _userData['ID'] as int : null;
//     if (userJson != null) {
//       final Map<String, dynamic> userMap = json.decode(userJson);
//       print("userMap$userMap");
//       setState(() {
//         _userData = userMap['user'];
...
//   try {
//     final SharedPreferences prefs = await SharedPreferences.getInstance();
//     final int? storedOrganizationId = prefs.getInt('ID');  
//     final String? adminname = prefs.getString('AdminName');
...
//       organizationID = prefs.getInt('ID');
//       organizationName = prefs.getString('orgName').toString();
...
//                         // Perform action with the selected value
//                         // _performActionWithSelectedValue(newValue);
//                       });
//                     },
...
import 'dart:convert';
import 'dart:io';
// import 'package:educationgame/screen/Admin/Function/admin.dart';
// import 'package:educationgame/screen/Admin/UI/admin.dart';
...
import '../../Teacher/UI/teacher.dart';
import '../Function/admin.dart';
import 'admin.dart';
...
class _DashboardState extends State<AdminDashboard> {
  ...
  bool _validateAndSubmit() {
    ...
    if (_usernameError.isEmpty &&
      ...
      printdata();
        var response=printdata();
        // var resbody=response.statusCode;
        print(1);
        // print(response.data)
...
//                   // Refresh data in AdminPage after successful registration
//                   adminFunction.refreshData();
//                 },
//                 child: Text('OK'),
//               ),
...
// }
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // Show error dialog if the registration failed
      _showErrorDialog('Failed to register: ${response.body}');
    }
  } catch (e) {
    // Show error dialog if there was an exception
...
void _showErrorDialog(String errorMessage) {
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text('Error'),
        content: Text("Failed to Insert Enter the Correct Data"),
        actions: <Widget>[
          TextButton(
            onPressed: () {
              Navigator.of(context).pop(); // Close the dialog
  ...
  Future<void> _retrieveLoginData() async {
    ...
    if (organizationID != null) {
      fetchOrganisationDetails(organizationID!);
    } else {
...
//     final String password = _passwordController.text;
//     final String registerNumber = _registerNumberController.text;
//     final String classlevel = _classController.text;  // Use classlevel1 from SharedPreferences if available
//     final String role = _selectedValue ?? '';  // Handle null for role
...
//     //     'OrganizationID': organisationId,  // Send the organization ID
//     //     'role': role,
//     //     'class': classlevel,  // Send the class level
//     //     'createdby': loginId,
//     //   }),
//     // );
//     final response = await http.post(
...
//     registerNumber = prefs.getString('registerNumber');
//     organizationID = prefs.getInt('id');
//     // organizationName = prefs.getString('Name');
//   });
...
//                 onPressed: () {
//                   printdata();
//                   //  _updateTextFieldValue(prefs.getString('class'));
//                   Navigator.of(context).pop();
//                 },
//                 child: Text('Submit'),
//               ),
...
_showPopup();                      },
                      icon:Tooltip(
        message: "Add Teacher", // Hover text
